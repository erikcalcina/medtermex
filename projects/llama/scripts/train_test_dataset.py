...
def main(args):
    ...
    file_tuples = [
        (txt, txt[:-4] + ".ann") for txt in txt_files if txt[:-4] + ".ann" in ann_files
    ]
    ...
    for x in range(10):
        for txtAnnPair in train_file_tuples:
            ...
            allAnnLines = [
                re.split(r"\t+", tag.rstrip("\t"))
                for tag in annfile
                if tag[0][0].startswith(("T"))
            ]
            ...
            # Adding to dataframe (input (raw text), output (correct json),
            # entities to extract (our keys divided by "," and ending with "."))
            df.loc[len(df.index)] = [
                Txtfile,
                json.dumps(
                    dict((k, annDict[k]) for k in keys if k in annDict),
                    ensure_ascii=False,
                ),
                ", ".join(keys) + ".",
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default=None, help="path to dataset directory"
    )
    parser.add_argument(
        "--train_save",
        type=str,
        default=None,
        help="path to saving training dataset (name it too)",
    )
    parser.add_argument(
        "--test_save",
        type=str,
        default=None,
        help="path to saving test dataset (name it too)",
    )
