...
def main(args):
    ...
    for example in tqdm(data, desc="Processing examples"):
        token_info = []
        ws_generator = model.data_processor.words_splitter(example["text"])
        for token_idx, token in enumerate(ws_generator):
            token_info.append((token[0], token[1], token[2], token_idx))
        ...
        gliner_example = {"tokenized_text": tokenized_text, "ner": gliner_entities}
        gliner_data.append(gliner_example)
